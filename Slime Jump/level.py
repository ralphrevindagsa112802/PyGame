...
class Bonus(Sprite):
	...
	def __init__(self, parent:Sprite,color=config.MINT,
...
class Platform(Sprite):
	...
	def __init__(self, x:int, y:int, width:int, height:int,
		...
		self._image = pygame.image.load('data/image/platform.png')  # Replace with the actual image file
		self._image = pygame.transform.scale(self._image, (width, height))
		
		self.rect = self._image.get_rect()  # Create the rect attribute based on the image size
		self.rect.x = x
		self.rect.y = y
    ...
    # Define the getter method for the 'image' property
	def get_image(self):
		return self._image

    # Use property to create a getter and setter for 'image'
